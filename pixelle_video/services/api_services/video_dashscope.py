...
Supports image-to-video for models such as wan2.7-i2v and wan2.6-i2v-flash
"""
...
class DashscopeVideoClient:
    ...
    def generate_video(
        ...
        if not video_url:
            ...
            if not task_id:
                raise RuntimeError(
                    "Wanxiang video API returned neither video_url nor task_id, cannot query the result: "
            ...
            if not video_url:
                raise RuntimeError(
                    "Wanxiang video task completed but still returned no video_url: "
...
if __name__ == "__main__":
    ...
    PROMPT = "A woman hands a report to a man; the man reads the data on the report clearly and shows a satisfied smile, office background, realistic style, high-definition detail. Background music: upbeat electronic music with a strong rhythm, suitable for an office environment."
    ...
    if not ak:
        print("✗ DASHSCOPE_API_KEY not set, please check the .env configuration")
        sys.exit(1)
