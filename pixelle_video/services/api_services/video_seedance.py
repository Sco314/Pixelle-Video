...
class SeedanceVideoClient:
    ...
    Supports image-to-video, using an asynchronous submit task -> poll -> download flow
    """
    ...
    def generate_video(
        ...
        **kwargs
        ...
        Args:
            prompt: prompt
            image_path: local path of the input image; when empty, text-to-video is used
    ...
    def _submit_task(self, prompt: str, image_path: Optional[str], model: str, duration: int, **kwargs) -> str:
        # Updated endpoint path per the Seedance 2.0 docs
        ...
        # Merge other optional parameters (such as seed, watermark)
    ...
    def _poll_until_done(self, task_id: str, max_polls: int = 120, interval: int = 5) -> str:
        # Correspondingly updated query endpoint path
        ...
        for i in range(max_polls):
            ...
            if status == "succeeded":
                ...
                if not video_url:
                    raise RuntimeError(f"Seedance task succeeded but returned no video URL: {data}")
                return video_url
...
if __name__ == "__main__":
    ...
    PROMPT = "A girl hands a financial report to a boy, and the boy weeps with joy after seeing it"
    ...
    if not api_key:
        print("✗ ARK_API_KEY not set, please check the .env configuration")
        sys.exit(1)
