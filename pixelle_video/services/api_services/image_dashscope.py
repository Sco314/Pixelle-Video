...
class DashScopeClient:
    ...
    def __init__(self, api_key=None, base_url=None, local_proxy=None):
        ...
        # Use the China (Beijing) region API by default; fall back to the default address if the env var or argument is not set
...
if __name__ == "__main__":
    ...
    # Text-to-image
    print("\n=== Text-to-image test ===")
    prompt = "An orange cat lying on a sunlit windowsill, watercolor style"
    for model in MODELS:
        print(f"\nPrompt: {prompt}")
        print(f"model: {model}")
        os.makedirs(save_dir, exist_ok=True)
    ...
    prompt = "Based on this image, add some drifting cherry blossom petals, rendered in watercolor style"
    for model in MODELS:
        print(f"\nPrompt: {prompt}")
        print(f"model: {model}")
        os.makedirs(save_dir, exist_ok=True)
